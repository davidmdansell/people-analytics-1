...
print(book3_data.head())

data = book3_data.copy()
...
# Visualizing the delta between demand and forecast by country
plt.figure(figsize=(14, 8))

plt.bar(index, country_data['delta demand vs forecast'], color='red')

plt.xlabel('Country')
plt.ylabel('Delta')
plt.title('Delta Between Hiring Demand and Forecast by Country')
plt.xticks(index, country_data['Unnamed: 0'], rotation=45)
plt.tight_layout()
